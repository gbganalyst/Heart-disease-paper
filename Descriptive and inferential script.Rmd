...
install.load::install_load(c("tidyverse", "readxl", "writexl", "inspectdf", "caret", "janitor", "infer"))
...
p <- cleveland_statlog_response %>% 
  ggplot(aes(x = heart_disease_status , y = n, col= heart_disease_status))+ geom_segment(aes(xend = heart_disease_status, yend = 0),size = 1.5, lineend = "square", show.legend = FALSE) +
  geom_point(size = 10, show.legend = FALSE)+ facet_wrap(~data)+ 
  scale_colour_manual(values = c("green", "red"))+
  labs(x = 'Heart disease status', y = 'Number of patients') +
  geom_text(aes(label = scales::percent(pct)), vjust= -0, size=2.5, colour = 'black')+
  theme(axis.title.y = element_text(size = 12, face ='bold'), axis.title.x = element_text(size = 12, face='bold'), axis.text.x = element_text(vjust = 0.3, face = "bold"))

ggsave(p, "Heart-disease-data-analysis/Exploratory-data-analysis/Cleveland-Statlog.png", width = 6.74, height = 4.54)
```
